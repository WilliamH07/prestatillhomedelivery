    ...
    public function initContent()
    {
		...
		switch ($action) {
			...
			case 'processCarrier':
				...
				// On check ici la distance de l'adresse de livraison du customer par rapport au shop ?
				$phd_config = new PrestatillHomeDeliveryConfiguration();
				$address_valid = $phd_config->getValidStores(true, $id_carrier);
				
				if(empty($address_valid))
				{
					$address_valid = $this->module->l('Youre delivery address seems not to be within the delivery zone. Please check your address or select another carrier.', 'validateordercarrier');
				}
				else if(isset($address_valid[0]) && isset($address_valid[0]['distance']))
				{
					$context->cookie->__set('distance', $address_valid[0]['distance']);
				}
