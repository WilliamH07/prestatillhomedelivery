...
class PrestatillHomeDelivery extends Module
{
    ...
    public function processConfiguration()
    {
        if (Tools::isSubmit('submitParameters')) {
			...
			// 3.1.0 - Zones API Key
			Configuration::updateValue('PRESTATILL_HD_ZONES_API_KEY',Tools::getValue('PRESTATILL_HD_ZONES_API_KEY'));
			// 3.1.1 - Geocoding API Keys
			Configuration::updateValue('GEOCODE_MAPS_API_KEY',Tools::getValue('GEOCODE_MAPS_API_KEY'));
    ...
    public function assignConfiguration()
    {
			...
			// 3.1.1 : Geocoding API Keys
            $this->context->smarty->assign('geocode_maps_api_key', Configuration::get('GEOCODE_MAPS_API_KEY'));
            $this->context->smarty->assign('google_maps_api_key', Configuration::get('GOOGLE_MAPS_API_KEY'));
    }
    ...
    public function getContent()
    {
		...
		$stores_raw = $phd_config->getValidStores();
		$stores = $this->reorderStores($stores_raw);
        ...
        $this->context->smarty->assign(array(
			...
			// 3.0.0
			'base_dir' => $base_dir,
			// 3.1.0 - Zones management
			'zones_enabled' => Configuration::get('PRESTATILL_HD_USE_ZONES'),
			'zones_api_key' => Configuration::get('PRESTATILL_HD_ZONES_API_KEY'),
			'stores_array' => !empty($stores_raw) ? array_values($stores_raw) : [],
			'carriers_array' => Carrier::getCarriers((int)$this->context->language->id, false, false, false, null, Carrier::ALL_CARRIERS),
        ));
