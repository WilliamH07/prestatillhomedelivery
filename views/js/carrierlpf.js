...
$(document).ready(function() {
   ...
   $('body').on('click tap', '#hd_list_creneau.viewList td span', function() {
        if (!$(this).hasClass('disabled')) {
            ...
            var theDay = $(this).data('date');

            // Réinitialise les sélecteurs
            $('#creneau_from, #creneau_to').children('option:first').prop('selected', true);

            // Affiche uniquement les options pertinentes
            showOptions('creneau_from', theDay);
            showOptions('creneau_to', theDay);
        }
    });
    
    $('body').on('change', '#hd_creneau #creneau_to', function() {
        $('#hd_creneau td').removeClass('selected');
        $('#hd_box').attr('data-creneau', '1');
        $.ajax({
            url: $('#hd_dispo').data('url'),
            action: 'assignSlotLPF',
            type: 'json',
            method: 'post',
            data: {
                slot: $('#creneau_to option:selected').data(),
                creneau_from: $('#creneau_from').val()+':00',
                creneau_to: $('#creneau_to').val()+':00',
    ...
    function hideOptions(selectId) {
        ...
        // Cloner et stocker toutes les options sauf celles avec le texte '--'
        savedOptions[selectId] = $select.children('option').filter(function() {
            return $(this).text() !== '--';
        }).clone(true).toArray();
    
        // Supprimer les options clonées, en laissant les options spéciales intactes
        $select.children('option').filter(function() {
            return $(this).text() !== '--';
        }).remove();
    }

    // Fonction pour montrer certaines options
    function showOptions(selectId, theDay) {
        var $select = $('#' + selectId);
    
        // Supprimer uniquement les options dynamiques, en laissant les options spéciales
        $select.children('option').filter(function() {
            return $(this).text() !== '--';
        }).remove();
    
        // Ajouter les options sauvegardées qui correspondent à la condition
        $.each(savedOptions[selectId], function() {
            var $option = $(this);
            if ($option.data('date') === theDay && $option.hasClass('dispo')) {
                $select.append($option.clone(true));
            }
        });
    }
    ...
    // LPF
    // Sauvegarder les options initiales de creneau_to
    //allToOptions = $('#hd_creneau #creneau_to option').clone();

    $('body').on('change', '#hd_creneau #creneau_from', function() {
    
        let min_hour = $(this).find('option:selected').data('hour');
        let lpf_slot_min_duration = parseInt($('#lpf_slot_min_duration').val(), 10);
        let minTime = new Date('01/01/2000 ' + min_hour);
    
        minTime.setMinutes(minTime.getMinutes() + lpf_slot_min_duration);

        // Restaurer toutes les options initiales
        //$('#hd_creneau #creneau_to').empty().append(allToOptions.clone());
    
        console.log("Options avant le filtrage :", $('#hd_creneau #creneau_to option').length);
    
        // Filtrer les options basées sur la durée minimale du créneau
        $('#hd_creneau #creneau_to option').not('[value="--"]').each(function() {
            var optionValue = $(this).val();
            var optionTime = new Date('01/01/2000 ' + optionValue);
        
            if (optionTime < minTime) {
                console.log('Retrait de l\'option:', $(this).text());
                $(this).remove();
            }
        });
    
        // Réinitialiser la sélection de creneau_to
        //$('#hd_creneau #creneau_to').prepend('<option value="--">--</option>'); // Ajouter l'option '--' si nécessaire
        $('#hd_creneau #creneau_to').val($('#hd_creneau #creneau_to option:first').val());
    });
                ...
                success: function(data) {
                    ...
                    if (data.status == 'success') {
                        ...
                        if(active == 1) {
                            ...
                            allToOptions = $('#hd_creneau #creneau_to option').not('[value="--"]').clone();
                        }
    ...
    function _viewList(message) {
        ...
        console.log(message.hd_slot_min_duration);
        
        var sizeh = Object.keys(message.creneau).length;

        html += '<div class="creneau_text_box"><span>Entre</span><span>et</span></div>';

        html +='<tr><td>';
        if(message.increment > 0)
        {
            // DU
            html += '<select id="creneau_from" class="form-control">';
                html += '<option class="creneau indispo">--</option>';
            for (var h = 0; h < sizeh - 1; h++) {
                for (var i = 1; i <= size; i++) {
                    var additional_class = 'indispo';
                    html += '<option class="creneau ' + additional_class + '" data-date="'+message.days[i].dateen+'" data-id-day="' + message.days[i].id_day + '" data-datetime="' + message.days[i].dateen + ' ' + message.creneau[h] + '" data-hour="' + message.creneau[h] + '" data-date="' + message.days[i].date + '" data-cl_carence="' + message.days[i].cl_carence + '" data-hour_end="00:00:00" >';
                    html += '' + $.format.date("0000-00-00 " + message.creneau[h] + "", "H:mm") + '</option>';
                }
            ...
            // AU
            html += '<select id="creneau_to" class="form-control">';
                html += '<option class="creneau indispo">--</option>';
            for (var h = 0; h < sizeh - 1; h++) {
                for (var i = 1; i <= size; i++) {
                    var additional_class = 'indispo';
                    html += '<option class="creneau ' + additional_class + '" data-date="'+message.days[i].dateen+'" data-id-day="' + message.days[i].id_day + '" data-datetime="' + message.days[i].dateen + ' ' + message.creneau[h] + '" data-hour="' + message.creneau[h] + '" data-date="' + message.days[i].date + '" data-hour_end="00:00:00" >';
                    html += '' + $.format.date("0000-00-00 " + message.creneau[h + 1] + "", "H:mm") + '</option>';
                }
    ...
    function changeclass(message) {
        ...
        $('#hd_creneau #creneau_from .indispo').each(function() {
            for (var e = 1; e <= 7; e++) {
                //console.log([$(this).data('hour'), $(this).data('cl_carence')]);
                if (message.open[e] != undefined && $(this).data('idDay') == message.open[e].id_day && message.open[e].nonstop == true && $(this).data('hour') >= message.open[e].hour_open_am && $(this).data('hour') <= $(this).data('cl_carence')) {
                    $(this).removeClass('indispo');
                    $(this).addClass('dispo');
                } else if (message.open[e] != undefined && $(this).data('idDay') == message.open[e].id_day && message.open[e].nonstop == false && (($(this).data('hour') >= message.open[e].hour_open_am && $(this).data('hour') < message.open[e].hour_close_am) || ($(this).data('hour') >= message.open[e].hour_open_pm && $(this).data('hour') <= $(this).data('cl_carence')))) {
                    $(this).removeClass('indispo');
                    $(this).addClass('dispo');
                }
            }
        ...
        $('#hd_creneau #creneau_to .indispo').each(function() {
            for (var e = 1; e <= 7; e++) {
    ...
    function adjustListDisplay(message) {
        ...
        $($('#hd_creneau tbody tr option').get().filter(td=>td.dataset.datetime<=datetime)).removeClass('dispo').addClass('indispo');
        ...
        $('body').find('#hd_list_creneau tr td span').each(function(){
            ...
            if($('#hd_creneau tr td option[data-date="'+the_day+'"].dispo').length == 0)
        ...
        hideOptions('creneau_from');
        hideOptions('creneau_to');
    }
    ...
    $('body').on('click tap', '#hd_creneau_selected .changeSlot', function(){
       ...
       $('#hd_creneau tr td #creneau_to option:first').prop('selected', true);
       $('#hd_box').attr('data-creneau', 0);
    });
